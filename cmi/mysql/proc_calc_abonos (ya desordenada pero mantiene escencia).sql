...
begin
	...
	begin	
        if v_monto_total_uf <0 then begin -- abona a la cuenta de interes
			...
			if abs(v_monto_negativo_uf) >0 then
				insert into transacciones(tr_fecha,tr_cc_id,tr_tipo_transaccion,tr_moneda,tr_monto,tr_monto_uf,tr_descripcion) values(p_fecha,p_cc_id,2,'clp',126, v_monto_negativo_uf,'capital');
            end if;
			insert into transacciones(tr_fecha,tr_cc_id,tr_tipo_transaccion,tr_moneda,tr_monto,tr_monto_uf,tr_descripcion) values(p_fecha,p_cc_id,1,'clp',126, v_monto_total_uf,'capital');
		end; end if; 
     end;   
