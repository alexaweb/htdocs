...
begin
    ...
    declare v_monto_positivo_uf decimal(20,6);
    declare v_monto_negativo_uf decimal(20,6);
    ...
    select ifnull(sum(tr_monto_uf),0) into v_monto_positivo_uf from transacciones where  tr_cc_id = p_cc_id and tr_fecha = p_fecha and tr_monto_uf >0;
    select ifnull(sum(tr_monto_uf),0) into v_monto_negativo_uf from transacciones where  tr_cc_id = p_cc_id and tr_fecha = p_fecha and tr_monto_uf <0;
    -- select v_monto_total_uf;
   
        
	if v_interes_si >0 then
	begin	
        if v_monto_interes_uf <0 then begin -- abona a la cuenta de interes
			if abs(v_monto_interes_uf) < abs(v_interes_si) then begin
				insert into transacciones(tr_fecha,tr_cc_id,tr_tipo_transaccion,tr_moneda,tr_monto,tr_monto_uf,tr_descripcion) values(p_fecha,p_cc_id,2,'clp',123, v_monto_interes_uf,'interes');
			end;
            else begin 
                set v_interes_pago = -(v_interes_si);
				-- set v_monto_total_interes_uf = v_monto_total_interes_uf + v_interes_pago;
                set v_capital_pago = v_monto_interes_uf - v_interes_pago;
		...
		insert into transacciones(tr_fecha,tr_cc_id,tr_tipo_transaccion,tr_moneda,tr_monto,tr_monto_uf,tr_descripcion) values(p_fecha,p_cc_id,1,'clp',126, v_monto_capital_uf,'capital');
     end;   
	elseif v_interes_si<0 then -- v_interes_si <0
	 begin
		if v_monto_total_uf >0 then begin -- abona a la cuenta de interes
			...
			insert into transacciones(tr_fecha,tr_cc_id,tr_tipo_transaccion,tr_moneda,tr_monto,tr_monto_uf,tr_descripcion) values(p_fecha,p_cc_id,1,'clp',226, v_monto_total_uf,'capital');
		end; end if;
	 end;
	else
	 begin
