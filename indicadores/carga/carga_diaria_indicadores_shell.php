...
try {
	...
	$db = new PDO("mysql:host=$dbhost;dbname=$dbname", 'root', 'oSTI.sCA');
	$sql = "select * from indicadores.indicadores where fecha='{$fecha}' and codigo='uf';";
    //$sql = "call proc_display_indicadores($ano,$mes);";
	//echo $sql;
    $stmt_sql = $db->prepare($sql);
    $result = $stmt_sql->execute();
	$row = $stmt_sql->fetch();
    $valor_uf_temp = $row['valor'];
    }
...
if (is_null($valor_uf_temp))  // EJECUTO EL PROCESAMIENTO DE IR A BUSCAR INDICADORES Y CARGAR LA BD
{
		  
// PRIMERO BORRO LOS INDICADORES ASI ELIMINO LA TAB DEL DIA ANTERIOR
try {
	$db = new PDO("mysql:host=$dbhost;dbname=$dbname", 'root', 'oSTI.sCA');
...
try {
	$db = new PDO("mysql:host=$dbhost;dbname=$dbname", 'root', 'oSTI.sCA');
	$sql = "delete from indicadores.indicadores where fecha='{$ayer}' and codigo='tabuf360';";
    //$sql = "call proc_display_indicadores($ano,$mes);";
	//echo $sql;
    $stmt_sql = $db->prepare($sql);
    $result = $stmt_sql->execute();
...
} // TERMINO EL PROCESAMIENTO DE IR A BUSCAR INDICADORES Y CARGAR LA BD
