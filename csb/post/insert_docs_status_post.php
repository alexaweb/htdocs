<?php
    ...
    if(!empty($_POST))
    {
        ...
        try
        {
            $sql = "call proc_insert_status_docs('{$item_id}','{$status_fecha}','{$status_detalle}');";
            //echo $sql;
			//die;
            $stmt = $db->prepare($sql);
            $result = $stmt->execute();
        ...
        header("Location: ../display_status_docs.php?item_id=$item_id&item_descripcion=$item_descripcion");
        
        // Calling die or exit after performing a redirect using the header function
        // is critical.  The rest of your PHP script will continue to execute and
        // will be sent to the user if you do not die or exit.
        die("Redirecting to display_status_docs.php");
    }
