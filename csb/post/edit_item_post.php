<?php
    ...
    if(!empty($_POST))
    {
        ...
        try
        {
            $sql = "call proc_update_item('{$item_id}','{$item_buque}','{$item_descripcion}','{$item_origen}','{$item_proveedor}','{$item_cotizacion}','{$item_factura}','{$item_fecha_rfq}','{$item_bl_awb}','{$item_container}','{$item_carrier}','{$item_status}','{$item_thc}');";
            //echo $sql;
			//die;
            $stmt = $db->prepare($sql);
            $result = $stmt->execute();
